...
exports.init=function(root, options){
    ...
    env=(env==='production'||env==='prod') ? 'production' : 'development';
    options.env=env;
    ...
    for (i in preset) if (preset.hasOwnProperty(i) && i!=='filters')
        options[i] = preset[i];
    ...
    options.addHash       = options.addHash!==false;//||true;
    options.hashedMaxAge  = options.hashedMaxAge || 365 * 24 * 60 * 60 * 1000;
    options.hashParName   = options.hashParName || '_fs_hash';
...
exports.addHash=function(file,hash){
    if(this.options.addHash && hash)
        file+=(file.indexOf('?')===-1 ? '?' : '&')+this.options.hashParName+'='+hash;
    return file;
};
exports.answer=function(req,res,path,options, next, url){
    ...
    self.makeFile(path,function(err,entry)
    {
        ...
        if(err){
            ...
            var maxAge=(url.indexOf('?'+options.hashParName+'=')!==-1 || url.indexOf('&'+options.hashParName+'=')!==-1) ? options.hashedMaxAge : options.maxAge;
